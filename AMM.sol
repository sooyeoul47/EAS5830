...
contract AMM is AccessControl{
	...
	*/
	function tradeTokens( address sellToken, uint256 sellAmount ) public {
		...
		uint256 sellQuantity = sellToken == tokenA ? qtyA : qtyB;
    	uint256 buyQuantity = sellToken == tokenA ? qtyB : qtyA;

		uint256 sellAmountAfterFee = sellAmount * (10000 - feebps) / 10000;
    	uint256 newSellQuantity = sellQuantity + sellAmountAfterFee;

		uint256 newBuyQuantity = invariant / newSellQuantity;
    	uint256 swapAmt = buyQuantity - newBuyQuantity;
		...
		sellTokenInstance.transferFrom(msg.sender, address(this), sellAmount);
    	buyTokenInstance.transfer(msg.sender, swapAmt);
